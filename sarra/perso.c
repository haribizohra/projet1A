...
void initialiserPerso(perso *p,int player)  // pointeur khatr bch tbadl fih 
{
	p->image[0]=IMG_Load("delta.png");
	p->image[1]=IMG_Load("alpha.png");
...
void deplacementPerso(SDL_Event event,perso *p)
{
	...
	switch(event.type)
	{
		...
		case SDL_MOUSEMOTION:
			if(event.motion.x > p->pos.x + 40)  // w 80   
			    p->mvt=1;
			else p->mvt=0;
	   	break;
	...
	if(p->mvt != p->previousmvt) // changer le mvt
	{
		p->frame[p->numPerso].x=0;  // ki ykos fi west l mvt yarjaa mel louel lmta l mvt lekher
		p->pos.y=347; //?
		p->previousmvt = p->mvt;
	}
